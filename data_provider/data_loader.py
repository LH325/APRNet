...
def ts_to_image(data, img_size=96, enc = 7):
    ...
    video = np.zeros((enc, img_size, img_size),dtype=np.uint8)
    ...
    for i in range(n):
        ...
        if ts.max() != ts.min():
            y_coords = (ts - ts.min()) / (ts.max() - ts.min())
            y_coords = np.round(y_coords * (img_size - 1)).astype(int)
        else:
            y_coords = np.full_like(ts, img_size // 2, dtype=int)

        gray_values = (y_coords / (img_size - 1) * 255).astype(np.uint8)

        for x in range(img_size):
            y = img_size - 1 - y_coords[x]
            image[y, x] = gray_values[x]
        mean = image.mean()
        std = image.std()
        img_norm = (image - mean) / (std + 1e-8)
        video[i] = img_norm
